...
Last month I spent an evening debugging RGB lighting on my desktop. This is not a serious problem, and I knew that at the time. But the bug turned out to be one I keep meeting in production LLM systems wearing a different hat.
...
The fix wasn't cleverer code. It was refusing to trust the tool's own account of itself. Five changes, each with a direct analog in harness design.
...
The return value is derived from observed state, not from the fact that a write didn't throw. Return what is *now true*, not what you *attempted*.
